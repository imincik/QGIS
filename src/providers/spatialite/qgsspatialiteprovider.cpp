    ...
    , mGotSpatialiteVersion( false )
{
  ...
  if ( lyr != NULL )
  {
    ...
    /*
     loadFieldsAbstractInterface use 'vector_layers_field_infos' table to guess data types of all not-geometry fields
     and this table is updated by calling UpdateLayerStatistics function. When there is no data available, it will use
     TEXT data type instead of INTEGER or DOUBLE, so it is better to use loadFields method.
    */
    loadFields();
    gaiaFreeVectorLayersList( list );
    alreadyDone = true;
